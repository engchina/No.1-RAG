...
IMAGE_QA_PROMPT_TEMPLATE_V1 = """## 厳格なコンテキストQAシステムの実行規則
...
IMAGE_QA_PROMPT_TEMPLATE_V2 = """--ロール--
Strict Context QA

--タスク--
画像にある情報のみをもとに簡潔に質問に答えてください（回答は日本語で）。
...
IMAGE_QA_PROMPT_TEMPLATE_V3 = """## 厳格なコンテキストQAシステムの実行規則

### 基本原則
1. 提供された画像を唯一の情報源として使用
2. 画像に存在しない情報は一切使用禁止
3. 推測・解釈・あなた自身の知識・外部知識の追加禁止
4. 回答は日本語で
...
2. 画像に存在しない情報の使用は一切禁止する  
3. 推測・解釈・自身の知識・外部知識の追加は不可とする  
4. 応答は日本語で行うこと  

### 回答生成規則
- **三段階出力形式**:  
  1. 画像解析: 画像内の情報を解析する  
  2. 根拠: 応答の裏付けとなる画像内の具体的要素を提示する  
  3. 回答: 質問に対し簡潔に応答する  
- **完全一致**: 質問内容と画像内容が完全に一致しない場合は応答不可  
- **時系列処理**: 複数の日付が存在する場合、最新情報を優先する  
