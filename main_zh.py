...
async def chat_stream(system_text, query_text, llm_answer_checkbox):
    if not llm_answer_checkbox or len(llm_answer_checkbox) == 0:
        raise gr.Error("请选择 LLM 模型")
    if not query_text:
        raise gr.Error("请输入用户消息")
...
def create_oci_cred(user_ocid, tenancy_ocid, fingerprint, private_key_file):
    ...
    if not user_ocid:
        raise gr.Error("请输入 User OCID")
    if not tenancy_ocid:
        raise gr.Error("请输入 Tenancy OCID")
    if not fingerprint:
        raise gr.Error("请输入 Fingerprint")
    if not private_key_file:
        raise gr.Error("请输入 Private Key")
...
def create_cohere_cred(cohere_cred_api_key):
    if not cohere_cred_api_key:
        raise gr.Error("请输入 Cohere API Key")
...
def create_openai_cred(openai_cred_base_url, openai_cred_api_key):
    if not openai_cred_base_url:
        raise gr.Error("请输入 OpenAI Base URL")
    if not openai_cred_api_key:
        raise gr.Error("请输入 OpenAI API Key")
    ...
    load_dotenv(find_dotenv())
    gr.Info("OpenAI API Key 的设置已经完成")
...
def create_claude_cred(claude_cred_api_key):
    if not claude_cred_api_key:
        raise gr.Error("请输入 Claude API Key")
    ...
    load_dotenv(env_path)
    gr.Info("Claude API Key 的设置已经完成")
...
def create_langfuse_cred(langfuse_cred_secret_key, langfuse_cred_public_key, langfuse_cred_host):
    if not langfuse_cred_secret_key:
        raise gr.Error("请输入 Langfuse Secret Key")
    if not langfuse_cred_public_key:
        raise gr.Error("请输入 Langfuse Public Key")
    if not langfuse_cred_host:
        raise gr.Error("请输入 Langfuse Host")
...
ORDER 
        BY dc.doc_id
            ...
            return (gr.Textbox(value=query_sql_output.strip()), gr.Markdown("**搜索结果数量**: " + str(
                docs_dataframe.shape[0]) + "   |   **搜索关键词**: (" + str(len(search_texts)) + ")[" + ', '.join(
                search_texts) + "]", visible=True),
                    gr.Dataframe(value=docs_dataframe, wrap=True,
                                 headers=["NO", "CONTENT", "EMBED_ID", "SOURCE", "DISTANCE", "SCORE", "KEY_WORDS"],
...
def generate_download_file(search_result, llm_answer_checkbox, llm_evaluation_checkbox,
    ...
    if llm_evaluation_checkbox:
        ...
        standard_answer_text = ""
    df1 = pd.DataFrame({'查询': [query_text], '标准回答': [standard_answer_text]})
    ...
    df3 = pd.DataFrame({'LLM 模型': ["cohere/command-r", "cohere/command-r-plus", "openai/gpt-4o", "openai/gpt-4",
                                       "claude/opus", "claude/sonnet", "claude/haiku"],
                        'LLM 消息': [command_r_response, command_r_plus_response, openai_gpt4o_response,
                                           openai_gpt4_response, claude_3_opus_response, claude_3_sonnet_response,
                                           claude_3_haiku_response],
                        'LLM 评价': [command_r_evaluation, command_r_plus_evaluation, openai_gpt4o_evaluation,
                                     openai_gpt4_evaluation, claude_3_opus_evaluation, claude_3_sonnet_evaluation,
                                     claude_3_haiku_evaluation]})
...
with gr.Blocks(css=custom_css) as app:
    gr.Markdown(value="# RAG 精度提高助手", elem_classes="main_Header")
    gr.Markdown(value="### LLM&RAG 验证和评价工具", elem_classes="sub_Header")
    with gr.Tabs() as tabs:
        with gr.TabItem(label="环境设置") as tab_setting:
            with gr.TabItem(label="OCI GenAI 的设置*") as tab_create_oci_cred:
                with gr.Accordion(label="使用的 SQL", open=False) as tab_create_oci_cred_sql_accordion:
                ...
                with gr.Row():
                    with gr.Column():
                        tab_create_oci_clear_button = gr.ClearButton(value="清除")
                    with gr.Column():
                        tab_create_oci_cred_button = gr.Button(value="设置/重新设置", variant="primary")
                with gr.Accordion(label="OCI GenAI 的测试", open=False) as tab_create_oci_cred_test_accordion:
                    with gr.Row():
                        with gr.Column():
                            tab_create_oci_cred_test_query_text = gr.Textbox(label="文本", lines=1, max_lines=1,
                                                                             value="你好")
                    with gr.Row():
                        with gr.Column():
                            tab_create_oci_cred_test_vector_text = gr.Textbox(label="向量", lines=10, max_lines=10,
                                                                              autoscroll=False)
                    with gr.Row():
                        with gr.Column():
                            tab_create_oci_cred_test_button = gr.Button(value="测试", variant="primary")
            with gr.TabItem(label="创建表*") as tab_create_table:
                with gr.Accordion(label="使用的 SQL", open=False) as tab_create_table_sql_accordion:
                    tab_create_table_sql_text = gr.Textbox(label="SQL", show_label=False, lines=25, max_lines=50,
                                                           autoscroll=False, interactive=False,
                                                           show_copy_button=True)
                with gr.Row():
                    with gr.Column():
                        tab_create_table_button = gr.Button(value="创建/重新创建", variant="primary")
            with gr.TabItem(label="Cohere 的设置(可选)") as tab_create_cohere_cred:
                with gr.Row():
                    with gr.Column():
                        tab_create_cohere_cred_api_key_text = gr.Textbox(label="API Key*", lines=1, interactive=True)
                with gr.Row():
                    with gr.Column():
                        tab_create_cohere_cred_button = gr.Button(value="设置/重新设置", variant="primary")
            with gr.TabItem(label="OpenAI 的设置(可选)") as tab_create_openai_cred:
                with gr.Row():
                    with gr.Column():
                ...
                with gr.Row():
                    with gr.Column():
                        tab_create_openai_cred_button = gr.Button(value="设置/重新设置", variant="primary")
            with gr.TabItem(label="Claude 的设置(可选)") as tab_create_claude_cred:
                with gr.Row():
                    with gr.Column():
                        tab_create_claude_cred_api_key_text = gr.Textbox(label="API Key*", lines=1, interactive=True)
                with gr.Row():
                    with gr.Column():
                        tab_create_claude_cred_button = gr.Button(value="设置/重新设置", variant="primary")
            with gr.TabItem(label="Langfuse 的设置(可选)") as tab_create_langfuse_cred:
                with gr.Row():
                    with gr.Column():
                ...
                with gr.Row():
                    with gr.Column():
                        tab_create_langfuse_cred_button = gr.Button(value="设置/重新设置", variant="primary")
        with gr.TabItem(label="LLM 评价") as tab_llm_evaluation:
            with gr.TabItem(label="和 LLM 聊天") as tab_chat_with_llm:
                with gr.Row():
                    with gr.Column():
                        tab_chat_with_llm_answer_checkbox_group = gr.CheckboxGroup(
                            ["cohere/command-r", "cohere/command-r-plus", "openai/gpt-4o", "openai/gpt-4",
                             "claude/opus", "claude/sonnet", "claude/haiku"],
                            label="LLM 模型*", value=[])
                with gr.Accordion(label="Command-R 消息",
                                  visible=False,
                                  open=True) as tab_chat_with_llm_command_r_accordion:
                    tab_chat_with_command_r_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                     lines=2,
                ...
                with gr.Accordion(label="Command-R+ 消息",
                    ...
                    tab_chat_with_command_r_plus_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                          lines=2,
                                                                          autoscroll=True, interactive=False,
                                                                          show_copy_button=True)
                with gr.Accordion(label="OpenAI gpt-4o 消息",
                                  visible=False,
                                  open=True) as tab_chat_with_llm_openai_gpt4o_accordion:
                    tab_chat_with_openai_gpt4o_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                        lines=2,
                                                                        autoscroll=True, interactive=False,
                                                                        show_copy_button=True)
                with gr.Accordion(label="OpenAI gpt-4 消息",
                                  visible=False,
                                  open=True) as tab_chat_with_llm_openai_gpt4_accordion:
                    tab_chat_with_openai_gpt4_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                       lines=2,
                                                                       autoscroll=True, interactive=False,
                                                                       show_copy_button=True)
                with gr.Accordion(label="Claude 3 Opus 消息",
                                  visible=False,
                                  open=True) as tab_chat_with_llm_claude_3_opus_accordion:
                    tab_chat_with_claude_3_opus_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                         lines=2,
                                                                         autoscroll=True, interactive=False,
                                                                         show_copy_button=True)
                with gr.Accordion(label="Claude 3.5 Sonnet 消息",
                                  visible=False,
                                  open=True) as tab_chat_with_llm_claude_3_sonnet_accordion:
                    tab_chat_with_claude_3_sonnet_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                           lines=2,
                                                                           autoscroll=True, interactive=False,
                                                                           show_copy_button=True)
                with gr.Accordion(label="Claude 3 Haiku 消息",
                                  visible=False,
                                  open=True) as tab_chat_with_llm_claude_3_haiku_accordion:
                    tab_chat_with_claude_3_haiku_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                          lines=2,
                                                                          autoscroll=True, interactive=False,
                                                                          show_copy_button=True)
                with gr.Accordion(open=False, label="系统・消息"):
                    #                     tab_chat_with_llm_system_text = gr.Textbox(label="系统・消息*", show_label=False, lines=5,
                    #                                                                max_lines=15,
                    ...
                    tab_chat_with_llm_system_text = gr.Textbox(label="系统・消息*", show_label=False, lines=5,
                                                               ...
                                                               value="""你是一个有用的助手。
请用与我相同的语言回复。
如果我切换了语言，也请相应地切换回复的语言。""")
                with gr.Row():
                    with gr.Column():
                        tab_chat_with_llm_query_text = gr.Textbox(label="用户・消息*", lines=2, max_lines=5)
                with gr.Row():
                    with gr.Column():
                        tab_chat_with_llm_clear_button = gr.ClearButton(value="消除")
                    with gr.Column():
        ...
        with gr.TabItem(label="RAG 评价", elem_classes="inner_tab") as tab_rag_evaluation:
            with gr.TabItem(label="Step-1.读取") as tab_load_document:
                with gr.Accordion(label="使用的 SQL", open=False) as tab_load_document_sql_accordion:
                    tab_load_document_output_sql_text = gr.Textbox(label="使用的 SQL", show_label=False, lines=10,
                                                                   autoscroll=False,
                                                                   show_copy_button=True)
                with gr.Row():
                    with gr.Column():
                ...
                with gr.Row(visible=False):
                    with gr.Column():
                        tab_load_document_page_count_text = gr.Textbox(label="页数", lines=1, interactive=False)
                with gr.Row():
                    with gr.Column():
                        tab_load_document_page_content_text = gr.Textbox(label="内容", lines=15, max_lines=15,
                                                                         autoscroll=False,
                                                                         show_copy_button=True, interactive=False)
                with gr.Row():
                    with gr.Column():
                        tab_load_document_file_text = gr.File(label="文件*",
                                                              ...
                                                              type="filepath")
                    with gr.Column():
                        tab_load_document_server_directory_text = gr.Text(label="服务器目录*",
                ...
                with gr.Row():
                    with gr.Column():
                        tab_load_document_load_button = gr.Button(value="读取", variant="primary")
            with gr.TabItem(label="Step-2.分割・向量化・保存") as tab_split_document:
                with gr.Accordion(label="使用的 SQL", open=False) as tab_split_document_sql_accordion:
                    tab_split_document_output_sql_text = gr.Textbox(label="使用的 SQL", show_label=False, lines=10,
                                                                    autoscroll=False,
                                                                    show_copy_button=True)
                with gr.Row():
                    tab_split_document_chunks_count = gr.Textbox(label="块数", lines=1)
                with gr.Row():
                    ...
                    with gr.Row():
                        with gr.Column():
                            tab_split_document_chunks_normalize_radio = gr.Radio(label="NORMALIZE",
                                                                                 ...
                                                                                 value="ALL",
                                                                                 visible=False,
                                                                                 info="Default value: ALL。文档をテキストに変換する際にありがちな問題について、自動的に前処理、後処理を実行し高品質なチャンクとして格納するために使用。NONE: 処理を行わない。ALL: マルチバイトの句読点をシングルバイトに正規化。OPTIONS: PUNCTUATION: スマート引用符、スマートハイフン、マルチバイト句読点をテキストに含める。WHITESPACE: 不要な文字を删除して空白を最小限に抑える例えば空白行はそのままに、余分な改行やスペース、タブを删除する。WIDECHAR: マルチバイト数字とローマ字をシングルバイトに正規化する")
                        with gr.Column():
                            tab_split_document_chunks_normalize_options_checkbox_group = gr.CheckboxGroup(
                                ...
                                choices=[
                                    ...
                                    ("WIDECHAR", "WIDECHAR")],
                                visible=False,
                                info="文档をテキストに変換する際にありがちな問題について、自動的に前処理、後処理を実行し高品質なチャンクとして格納するために使用。PUNCTUATION: スマート引用符、スマートハイフン、マルチバイト句読点をテキストに含める。WHITESPACE: 不要な文字を删除して空白を最小限に抑える例えば空白行はそのままに、余分な改行やスペース、タブを删除する。WIDECHAR: マルチバイト数字とローマ字をシングルバイトに正規化する")
                ...
                with gr.Row():
                    ...
                    with gr.Column():
                        tab_split_document_embed_save_button = gr.Button(
                            value="将数据向量化并保存",
                            variant="primary")
            with gr.TabItem(label="Step-3.删除(可选)") as tab_delete_document:
                with gr.Accordion(label="使用的 SQL", open=False) as tab_delete_document_sql_accordion:
                    tab_delete_document_delete_sql = gr.Textbox(label="生成的 SQL", show_label=False, lines=10,
                                                                autoscroll=False, show_copy_button=True)
                with gr.Row():
                    with gr.Column():
                        tab_delete_document_server_directory_text = gr.Text(label="服务器目录*",
                                                                            value="/u01/data/no1rag/")
                with gr.Row():
                    with gr.Column():
                        # doc_id_text = gr.Textbox(label="Doc ID*", lines=1)
                ...
                with gr.Row():
                    with gr.Column():
                        tab_delete_document_delete_button = gr.Button(value="删除", variant="primary")
            with gr.TabItem(label="Step-4.与文档聊天") as tab_chat_document:
                with gr.Row():
                    with gr.Column():
                ...
                with gr.Row():
                    with gr.Column():
                        tab_chat_document_question_embedding_model_checkbox_group = gr.CheckboxGroup(
                            ...
                            label="Embedding 模型*", value="cohere/embed-multilingual-v3.0", interactive=False)
                    with gr.Column():
                        tab_chat_document_reranker_model_radio = gr.Radio(
                            ["None", "cohere/rerank-multilingual-v3.0",
                             "cohere/rerank-english-v3.0"
                             ],
                            label="Rerank 模型*", value="None")
                with gr.Row():
                    with gr.Column():
                        tab_chat_document_top_k_slider = gr.Slider(label="相似性 Top-K*", minimum=1,
                                                                   maximum=100, step=1,
                                                                   info="Default value: 25。按照相似度距离较低的顺序（即相似度较高的顺序）提取前K件。",
                                                                   interactive=True, value=25)
                    with gr.Column():
                        tab_chat_document_threshold_value_slider = gr.Slider(label="相似性搜索阈值*",
                                                                             minimum=0.10,
                                                                             info="Default value: 0.55。只抽取相似度距离低于阈值的数据。",
                                                                             maximum=0.95, step=0.05, value=0.55)
                with gr.Row():
                    with gr.Column():
                        tab_chat_document_reranker_top_k_slider = gr.Slider(label="Rerank Top-K*", minimum=1,
                                                                            maximum=50, step=1,
                                                                            info="Default value: 5。按照Rerank Score的高到低的顺序，只提取前K件。",
                    ...
                    with gr.Row():
                        with gr.Column():
                            tab_chat_document_partition_by_k_slider = gr.Slider(label="Partition-By-K", minimum=0,
                                                                                maximum=20, step=1,
                                                                                interactive=True,
                                                                                info="Default value: 0。指定搜索对象的文档数量。0: 全部。1: 1个。2：2个。... n: n个。")
                        with gr.Column():
                            tab_chat_document_answer_by_one_checkbox = gr.Checkbox(
                                label="Highest-Ranked-One 文档回答",
                                value=False,
                                info="忽略其他所有文档，仅根据排名第一的文档进行回答。")
                    with gr.Row():
                        with gr.Column():
                            tab_chat_document_extend_first_chunk_size = gr.Slider(label="Extend-First-K", minimum=0,
                                                                                  ...
                                                                                  interactive=True,
                                                                                  value=0,
                                                                                  info="Default value: 0。DISTANCE计算不包括在内。指定扩展被检索到的块的数量。0: 不扩展。1: 扩展最初的1个。2: 扩展最初的2个。... n: 扩展最初的n个。")
                        with gr.Column():
                            tab_chat_document_extend_around_chunk_size = gr.Slider(label="Extend-Around-K", minimum=0,
                                                                                   maximum=50, step=2,
                                                                                   interactive=True,
                                                                                   value=2,
                                                                                   info="Default value: 2。DISTANCE计算不包括在内。指定扩展被检索到的块的数量。0: 不扩展。2: 扩展前后各1个，总共2个。4: 扩展前后各2个，总共4个。... n: 扩展前后各n/2个，总共n个。")
                    with gr.Row():
                        with gr.Column():
                            tab_chat_document_text_search_checkbox = gr.Checkbox(label="全文搜索", value=False,
                                                                                 info="文本检索仅限于原始查询，忽略通过查询扩展生成的查询。")
                        with gr.Column():
                            tab_chat_document_text_search_k_slider = gr.Slider(label="文本检索 Limit-K",
                                                                               minimum=1,
                                                                               maximum=10, step=1,
                                                                               value=6,
                                                                               info="Default value: 6。限制可用于文本检索的单词数量。")
                with gr.Row(visible=False):
                ...
                with gr.Row():
                    #                     tab_chat_document_system_message_text = gr.Textbox(label="系统・消息*", lines=15,
                    #                                                                        max_lines=20,
                    #                                                                        interactive=True,
                    #                                                                        visible=False,
...
-目标-
你的任务是将给定的答案与标准答案进行比较，并评估其质量。
请根据以下标准，以0到10的评分进行评估：

准确性（0表示完全不准确，10表示完全准确）
完整性（0表示完全不完整，10表示完全令人满意）
清晰度（0表示非常不清晰，10表示非常清晰）
简洁性（0表示非常冗长，10表示最为简洁）
评分后，请为每个评分提供简要的解释。
最后，请提供一个从0到10的总体评分，并对评估结果进行总结。
请使用和我消息相同的语言进行回复。如果我切换了语言，请相应地切换回复的语言。\n""")
                with gr.Row():
                    tab_chat_document_standard_answer_text = gr.Textbox(label="標準回答*", lines=2, interactive=True,
                                                                        visible=False)
                with gr.Accordion("文档*", open=True):
                    with gr.Row():
                        with gr.Column():
                ...
                with gr.Accordion("查询扩展", open=False):
                    with gr.Row():
                        ...
                        tab_chat_document_generate_query_radio = gr.Radio(
                            [("None", "None"), ("子查询", "Sub-Query"), ("类似查询", "RAG-Fusion"),
                             ("假定回答", "HyDE"), ("抽象化查询", "Step-Back-Prompting")],
                            label="由 LLM 生成？", value="None", interactive=True)
                    with gr.Row():
                ...
                with gr.Accordion(label="使用的 SQL", open=False) as tab_chat_document_sql_accordion:
                    tab_chat_document_output_sql_text = gr.Textbox(label="使用的 SQL", show_label=False, lines=25,
                ...
                with gr.Row() as tab_chat_document_searched_data_summary_row:
                    ...
                    with gr.Column(scale=2):
                        tab_chat_document_download_output_button = gr.DownloadButton(label="下载",
                                                                                     visible=False,
                ...
                with gr.Accordion(label="Command-R 消息",
                    ...
                    tab_chat_document_command_r_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                         lines=2,
                                                                         autoscroll=True, interactive=False,
                                                                         show_copy_button=True)
                    with gr.Accordion(label="评价结果", visible=False,
                                      open=True) as tab_chat_document_llm_command_r_evaluation_accordion:
                        tab_chat_document_command_r_evaluation_text = gr.Textbox(label="评价结果", show_label=False,
                                                                                 lines=2,
                                                                                 autoscroll=True, interactive=False,
                                                                                 show_copy_button=True)
                with gr.Accordion(label="Command-R+ 消息",
                    ...
                    tab_chat_document_command_r_plus_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                              lines=2,
                                                                              autoscroll=True, interactive=False,
                                                                              show_copy_button=True)
                    with gr.Accordion(label="评价结果", visible=False,
                                      open=True) as tab_chat_document_llm_command_r_plus_evaluation_accordion:
                        tab_chat_document_command_r_plus_evaluation_text = gr.Textbox(label="评价结果",
                                                                                      show_label=False,
                                                                                      lines=2,
                                                                                      autoscroll=True,
                                                                                      interactive=False,
                ...
                with gr.Accordion(label="OpenAI gpt-4o 消息",
                    ...
                    tab_chat_document_openai_gpt4o_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                            lines=2,
                                                                            autoscroll=True, interactive=False,
                                                                            show_copy_button=True)
                    with gr.Accordion(label="评价结果", visible=False,
                                      open=True) as tab_chat_document_llm_openai_gpt4o_evaluation_accordion:
                        tab_chat_document_openai_gpt4o_evaluation_text = gr.Textbox(label="评价结果", show_label=False,
                                                                                    lines=2,
                                                                                    autoscroll=True, interactive=False,
                                                                                    show_copy_button=True)
                with gr.Accordion(label="OpenAI gpt-4 消息",
                    ...
                    tab_chat_document_openai_gpt4_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                           lines=2,
                                                                           autoscroll=True, interactive=False,
                                                                           show_copy_button=True)
                    with gr.Accordion(label="评价结果", visible=False,
                                      open=True) as tab_chat_document_llm_openai_gpt4_evaluation_accordion:
                        tab_chat_document_openai_gpt4_evaluation_text = gr.Textbox(label="评价结果", show_label=False,
                                                                                   lines=2,
                                                                                   autoscroll=True, interactive=False,
                                                                                   show_copy_button=True)
                with gr.Accordion(label="Claude 3 Opus 消息",
                    ...
                    tab_chat_document_claude_3_opus_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                             lines=2,
                                                                             autoscroll=True, interactive=False,
                                                                             show_copy_button=True)
                    with gr.Accordion(label="评价结果", visible=False,
                                      open=True) as tab_chat_document_llm_claude_3_opus_evaluation_accordion:
                        tab_chat_document_claude_3_opus_evaluation_text = gr.Textbox(label="评价结果", show_label=False,
                                                                                     lines=2,
                                                                                     autoscroll=True, interactive=False,
                                                                                     show_copy_button=True)
                with gr.Accordion(label="Claude 3.5 Sonnet 消息",
                    ...
                    tab_chat_document_claude_3_sonnet_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                               lines=2,
                                                                               autoscroll=True, interactive=False,
                                                                               show_copy_button=True)
                    with gr.Accordion(label="评价结果", visible=False,
                                      open=True) as tab_chat_document_llm_claude_3_sonnet_evaluation_accordion:
                        tab_chat_document_claude_3_sonnet_evaluation_text = gr.Textbox(label="评价结果",
                                                                                       show_label=False,
                                                                                       lines=2,
                                                                                       autoscroll=True,
                                                                                       interactive=False,
                ...
                with gr.Accordion(label="Claude 3 Haiku 消息",
                    ...
                    tab_chat_document_claude_3_haiku_answer_text = gr.Textbox(label="LLM 消息", show_label=False,
                                                                              lines=2,
                                                                              autoscroll=True, interactive=False,
                                                                              show_copy_button=True)
                    with gr.Accordion(label="评价结果", visible=False,
                                      open=True) as tab_chat_document_llm_claude_3_haiku_evaluation_accordion:
                        tab_chat_document_claude_3_haiku_evaluation_text = gr.Textbox(label="评价结果",
                                                                                      show_label=False,
                                                                                      lines=2,
                                                                                      autoscroll=True,
                                                                                      interactive=False,
