        ...
        with gr.TabItem(label="LLM評価") as tab_llm_evaluation:
            with gr.TabItem(label="LLMとチャット") as tab_chat_with_llm:
                ...
                with gr.Accordion(open=False,
                                  label="画像ファイル(オプション) - Grok-4、Llama-4-Scoutモデルを利用する場合に限り、この画像入力が適用されます。"):
                    ...
                    with gr.Row():
                        with gr.Column():
                            tab_convert_document_convert_button = gr.Button(
                                value="ExcelをTextへ変換",
                                variant="primary")
                with gr.TabItem(label="MarkItDown", visible=False) as tab_convert_by_markitdown_document:
                    with gr.Row():
                        with gr.Column():
                    ...
                    with gr.Row():
                        with gr.Column():
                            tab_chat_document_single_image_processing_radio = gr.Radio(
                                ...
                                value="ファイル単位で処理",
                                visible=False,
                                info="Default value: ファイル単位で処理。1枚ずつ処理: 各画像を個別に分析。全画像まとめて処理: 全ての画像を一度に送信。ファイル単位で処理: 同一ファイルの画像をまとめて処理。ファイル単位で処理+最初・最後: 各ファイルの最初と最後の画像を含めて処理。ファイル単位で処理+最初・最後・前後画像: 各ファイルの最初と最後の画像及び検索された画像の前後を含めて処理。（画像が10枚を超えるか、合計サイズが大きい場合、エラーが発生するため、「画像 Limit-K」を調整するか、「画像処理方式」を変更してください。）"
                            )
