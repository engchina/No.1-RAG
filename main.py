                    ...
                    with gr.Row():
                        with gr.Column():
                            tab_chat_document_single_image_processing_radio = gr.Radio(
                                ...
                                value="ファイル単位で処理+最初・最後",
                                visible=False,
                                info="Default value: ファイル単位で処理+最初・最後。1枚ずつ処理: 各画像を個別に分析。全画像まとめて処理: 全ての画像を一度に送信。ファイル単位で処理: 同一ファイルの画像をまとめて処理。ファイル単位で処理+最初・最後: 各ファイルの最初と最後の画像を含めて処理。ファイル単位で処理+最初・最後・前後画像: 各ファイルの最初と最後の画像及び検索された画像の前後を含めて処理。（画像を拡張すると10枚を超えた場合にエラーが発生しますので、、'画像 Limit-K'を適切に調整してください。）"
                            )
